...
Write reusable functions while preserving type information. This lesson is part of **TypeScript Basics**. It is written for beginners, but it uses realistic development examples so the learning assistant can answer questions about actual code and workflow decisions.
...
function formatProfile(profile: UserProfile): string {
  return `${profile.displayName} has completed ${profile.completedLessons} lessons.`;
}
...
console.log(formatProfile(learner));
```
...
- "Explain Generics and Reusable Functions with a smaller example."
...
Generics and Reusable Functions is a foundation skill. Master it by reading examples, changing them carefully, and explaining your reasoning out loud.
