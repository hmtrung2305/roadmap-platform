...
const ROLES_CACHE_KEY =
  "skill-gap:career-roles";

const ROADMAPS_CACHE_PREFIX =
  "skill-gap:published-roadmaps";

const ROLES_CACHE_MS =
  5 * 60 * 1000;

const ROADMAPS_CACHE_MS =
  5 * 60 * 1000;
  ...
  (set, get) => ({
    ...
    selectRole: (role) => {
      const normalizedRole =
        normalizeCareerRole(role);

      const roleSlug =
        getRoleSlug(normalizedRole);
    ...
    loadRoadmaps: async ({
      ...
      const requestVersion =
        ++roadmapsRequestVersion;

      const cachedRoadmaps =
        get().roadmapsByRoleSlug[
          roleSlug
        ];
      ...
      try {
        ...
        set({
          error:
            getFriendlyApiErrorMessage(
              error,
              "Unable to load published roadmaps for this role.",
            ),
        });
      ...
      try {
        ...
        set({
          error:
            getFriendlyApiErrorMessage(
              error,
              "Unable to load skill checklist for this roadmap.",
            ),
        });
    ...
    toggleSkill: (skillId) => {
      ...
      set((state) => ({
        selectedSkillIds:
          state.selectedSkillIds.includes(
            normalizedSkillId,
          )
        ...
        const result =
          await guardedMutation(
            ...
            async () => {
              ...
              return normalizeSkillGapResult(
                response,
              );
            },
          );
