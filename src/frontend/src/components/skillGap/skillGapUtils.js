import { ASSESSMENT_LEVEL_STYLES, SKILL_GAP_PRIORITY_STYLES } from "./skillGapConstants";
...
export function getRuleDescription(group) {
  ...
  const rule = getSelectionRuleLabel(selectionType || group.completionRule || group.CompletionRule);
  const requiredCount = group.requiredCount ?? group.requiredSkillCount ?? 1;
  const total = group.totalSkillCount ?? group.totalSkills ?? group.skills?.length ?? 0;
...
export function isGroupCompleted(group, selectedNodeIds) {
  ...
  const matchedCount = skills.filter((skill) => selected.has(normalizeId(skill.nodeId || skill.skillId || skill.id || skill.slug))).length;
  const total = skills.length;
  const rule = getSelectionRuleLabel(group.selectionType || group.completionRule);

  if (rule === "ALL") return matchedCount === total && total > 0;
  if (rule === "COUNT") return matchedCount >= (group.requiredCount ?? group.requiredSkillCount ?? 1);
...
export function normalizeAssessmentLevel(level) {
  ...
  const levelName = getValue(level, "levelName", "LevelName", getValue(level, "name", "Name", "Assessment level"));
  const slug = getValue(level, "slug", "Slug", getValue(level, "levelSlug", "LevelSlug", ""));
...
export function normalizeAssessmentLevels(response) {
  return toArray(response).map(normalizeAssessmentLevel).filter((level) => level?.slug);
}
...
export function normalizeAssessmentGroups(response) {
  const groups = toArray(getValue(response, "groups", "Groups", getValue(response, "skillGroups", "SkillGroups", response)));
  ...
  return groups.map((group, index) => {
    ...
    const selectionType = getValue(group, "selectionType", "SelectionType", "choose_one");
    const requiredCount = getValue(group, "requiredCount", "RequiredCount", getValue(group, "requiredSkillCount", "RequiredSkillCount", null));
    const skills = toArray(getValue(group, "skills", "Skills")).map(normalizeAssessmentSkill);
...
function buildSelectedSkillNameMap(groups, selectedNodeIds) {
  ...
  toArray(groups).forEach((group) => {
    toArray(group.skills).forEach((skill) => {
      const nodeId = normalizeId(skill.nodeId || skill.skillId || skill.id || skill.slug);
...
export function normalizeSkillItems(value) {
  return toArray(value)
    .map((item) => {
      ...
      const nodeId = getValue(item, "nodeId", "NodeId", getValue(item, "skillId", "SkillId", getValue(item, "id", "Id", "")));
      const name = getValue(item, "name", "Name", getValue(item, "title", "Title", getValue(item, "slug", "Slug", "Unnamed skill")));
...
export function normalizeSkillGapResult(rawResult, context = {}) {
  ...
  const selectedNameMap = buildSelectedSkillNameMap(context.groups, context.selectedNodeIds);

  const groups = toArray(getValue(rawResult, "groups", "Groups")).map((group, index) => {
    const groupId = normalizeId(
      getValue(group, "groupId", "GroupId") ??
        getValue(group, "skillGroupId", "SkillGroupId") ??
    ...
    const selectionType = getValue(group, "selectionType", "SelectionType", "choose_one");
    const totalSkillCount = Number(getValue(group, "totalSkills", "TotalSkills", getValue(group, "totalSkillCount", "TotalSkillCount", 0)));
    const matchedSkillCount = Number(getValue(group, "matchedSkills", "MatchedSkills", getValue(group, "matchedSkillCount", "MatchedSkillCount", 0)));
    const matchingAssessmentGroup = toArray(context.groups).find((item) => normalizeId(item.groupId || item.skillGroupId) === groupId || item.groupName === getValue(group, "groupName", "GroupName"));
    const matchedSkillItems = normalizeSkillItems(getValue(group, "matchedSkillItems", "MatchedSkillItems", getValue(group, "alreadyHaveSkills", "AlreadyHaveSkills", [])));
    const matchedSkills = matchedSkillItems.length > 0
      ? matchedSkillItems
      : toArray(matchingAssessmentGroup?.skills)
        .filter((skill) => selected.has(normalizeId(skill.nodeId || skill.skillId || skill.id || skill.slug)))
        .map((skill) => ({
          nodeId: skill.nodeId || skill.skillId || skill.id || skill.slug,
          name: skill.name || selectedNameMap.get(skill.nodeId) || skill.slug,
    ...
    return {
      ...
      requiredSkillCount: getValue(group, "requiredCount", "RequiredCount", null),
      isCompleted: Boolean(getValue(group, "isCompleted", "IsCompleted", false)),
      matchedSkillItems,
      alreadyHaveSkills: matchedSkills,
      matchedSkills,
      suggestedSkills: normalizeMissingSkills(getValue(group, "missingSkills", "MissingSkills", getValue(group, "suggestedSkills", "SuggestedSkills", []))),
    };
  });

  const totalSkills = Number(getValue(rawResult, "totalSkills", "TotalSkills", groups.reduce((sum, group) => sum + group.totalSkillCount, 0)));
  const matchedSkills = Number(getValue(rawResult, "matchedSkills", "MatchedSkills", groups.reduce((sum, group) => sum + group.matchedSkillCount, 0)));
  const totalGroups = Number(getValue(rawResult, "totalGroups", "TotalGroups", groups.length));
  const completedGroups = Number(getValue(rawResult, "completedGroups", "CompletedGroups", 0));
  const missingSkills = Number(getValue(rawResult, "missingSkills", "MissingSkills", Math.max(totalSkills - matchedSkills, 0)));

  return {
    ...rawResult,
    careerRoleName: getValue(rawResult, "careerRoleName", "CareerRoleName", "Selected role"),
    levelName: getValue(rawResult, "levelName", "LevelName", "Assessment level"),
    ...
    totalGroups,
    completedGroups,
    missingGroups: Number(getValue(rawResult, "missingGroups", "MissingGroups", Math.max(totalGroups - completedGroups, 0))),
    groups,
    missingGroupList: groups.filter((group) => !group.isCompleted).map((group) => group.groupName),
  };
}
