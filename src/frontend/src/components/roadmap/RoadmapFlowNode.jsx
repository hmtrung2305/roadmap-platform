...
export default function RoadmapFlowNode({ data }) {
  ...
  return (
    <button
      type="button"
      className={[
        "group relative flex flex-col items-center justify-center gap-1.5 rounded-lg border border-[#B9D8CC] px-4 py-3 text-center shadow-sm transition-none",
    ...
    >
      ...
      <span
        className="absolute left-2 top-1.5 h-2.5 w-2.5 rounded-lg border border-[#B9D8CC]"
      ...
      {data.nodeType === "choice_group" && (
        <span className="rounded-lg border border-[#B9D8CC] bg-white px-2 py-0.5 text-[10px] font-extrabold tracking-[0.08em] text-[#18332D]">
