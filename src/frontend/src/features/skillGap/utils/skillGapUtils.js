...
export function normalizeRoadmapOptions(roadmaps) {
  return toArray(roadmaps)
    .map(normalizeRoadmapOption)
    .filter((item) => item?.roadmapId);
}

export function normalizeAssessmentSkill(skill) {
  if (!skill) return null;

  const skillId = normalizeId(
    pick(
      skill,
      [
        "skillId",
  ...
  const isSuggestedFromCompletedNodes = Boolean(
    pick(
      skill,
      [
  ...
  const completedNodeCount = toNumber(
    pick(
      skill,
      [
        "completedNodeCount",
...
export function normalizeAssessmentCategory(
  ...
  const calculatedMatchedSkills =
    skills.filter(
      (skill) => skill.isMatched,
    ).length;
  ...
  return {
    ...
    matchedSkills: toNumber(
      pick(
        category,
        ["matchedSkills", "MatchedSkills"],
        calculatedMatchedSkills,
      ),
      calculatedMatchedSkills,
    ),

    missingSkills: toNumber(
      pick(
        category,
        ["missingSkills", "MissingSkills"],
        Math.max(
          0,
          skills.length - calculatedMatchedSkills,
        ),
      ),
      Math.max(
        0,
...
export function getSuggestedSkillIds(
  categories,
) {
  return [
    ...new Set(
...
export function normalizeAssessmentResponse(
  ...
  return {
    ...
    categories:
      normalizeAssessmentCategories(response),
  };
}

export function normalizeSkillGapResult(
  response,
) {
  if (!response) return null;
  ...
  const calculatedTotalSkills =
    categories.reduce(
      (sum, category) =>
        sum + category.skills.length,
      0,
    );
...
export function normalizeSkillGapHistoryItem(
  item,
) {
  if (!item) return null;
...
export function normalizeSkillGapHistory(items) {
  return toArray(items)
    .map(normalizeSkillGapHistoryItem)
    .filter(Boolean);
}
...
export function getSelectionSummary(
  ...
  return {
    selectedCount: allSkills.filter(
      (skill) =>
        selectedSet.has(skill.skillId),
    ).length,
