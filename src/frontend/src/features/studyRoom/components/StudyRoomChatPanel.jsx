  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        ul: ({ children }) => (
          <ul className="my-2 list-disc space-y-1 pl-5 marker:text-[#2FA084]">
            {children}
          </ul>
        ),
        ol: ({ children }) => (
          <ol className="my-2 list-decimal space-y-1 pl-5 marker:font-extrabold marker:text-[#2FA084]">
            {children}
          </ol>
        ),
        ...
        pre: ({ children }) => (
          <pre className="my-2 overflow-hidden rounded-md bg-[#18332D] p-0">
            {children}
          </pre>
        ),
        table: ({ children }) => (
...
export default function StudyRoomChatPanel({
  ...
  const sendModuleChatMessage = useLearningModuleStore(
    (state) => state.sendModuleChatMessage,
  );
  const creditStatus = useAiCreditStore((state) => state.creditStatus);
  const isLoadingCredits = useAiCreditStore(
    (state) => state.isLoadingCreditStatus,
  );
  const loadCreditStatus = useAiCreditStore((state) => state.loadCreditStatus);
  const patchCreditStatus = useAiCreditStore(
    (state) => state.patchCreditStatus,
  );
  const invalidateCreditStatus = useAiCreditStore(
    (state) => state.invalidateCreditStatus,
  );
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const responseCreditStatus =
        response?.creditStatus || response?.aiCreditStatus;
