...
import { isSubmittedToday } from "../utils/studyRoomUtils";
import {
  clearQuizAttemptDraft,
  loadQuizAttemptDraft,
  saveQuizAttemptDraft,
} from "../utils/quizAttemptDraftStorage";
...
function getSelectedOptionId(answer) {
  return (
    ...
    answer?.SelectedOptionId ||
    answer?.skillModuleQuizOptionId ||
    answer?.SkillModuleQuizOptionId
  );
}
...
function hasAttemptQuestions(attempt) {
  return (
    Array.isArray(attempt?.quiz?.questions) && attempt.quiz.questions.length > 0
  );
}
...
export default function StudyQuiz({
  ...
  const loadQuizAttempts = useLearningModuleStore(
    (state) => state.loadQuizAttempts,
  );
  const startQuizAttempt = useLearningModuleStore(
    (state) => state.startQuizAttempt,
  );
  const loadQuizAttemptSession = useLearningModuleStore(
    (state) => state.loadQuizAttemptSession,
  );
  const loadQuizAttemptReview = useLearningModuleStore(
    (state) => state.loadQuizAttemptReview,
  );
  const submitQuizAttempt = useLearningModuleStore(
    (state) => state.submitQuizAttempt,
  );
  const attempts = useLearningModuleStore((state) =>
    state.getQuizAttempts(module?.skillModuleId),
  );
  const isLoadingAttempts = useLearningModuleStore((state) =>
    state.getQuizAttemptsLoading(module?.skillModuleId),
  );
  ...
  const submittedAttempts = attempts.filter(
    (item) => getAttemptStatus(item) === "submitted",
  );
  ...
  const resumeAttempt = async (attemptId) => {
    ...
    const data = await loadQuizAttemptSession(module.skillModuleId, attemptId, {
      force: true,
    });
  ...
  const startAttempt = async () => {
    ...
    try {
      ...
      if (inProgressAttemptId) {
        const didResume = await resumeAttempt(inProgressAttemptId).catch(
          () => false,
        );
      ...
      <div className="space-y-4 rounded-lg border border-[#B9D8CC] bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="flex items-center gap-2 text-xl font-black text-[#18332D]">
              <Trophy size={20} />{" "}
              {attempt.quiz?.title || quiz.title || "Module quiz"}
            </h2>
            <p className="mt-1 text-sm font-semibold text-slate-600">
              Attempt {attempt.attemptNo}. Answer every question before
              submitting.
            </p>
          </div>
          <ModuleButton
        ...
        {questions.map((question, index) => {
          ...
          return (
            <div
              key={questionId}
              className="rounded-lg border border-[#B9D8CC] bg-[#F7F1E8]/40 p-4"
            >
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-[#B9D8CC] bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          <QuizStat
            label="Passing score"
            value={`${quiz.passingScorePercent ?? 0}%`}
          />
          <QuizStat
        ...
        <div className="mt-5 flex justify-end">
          <ModuleButton
            ...
            disabled={isStarting || remainingAttempts === 0 || !canStartQuiz}
          >
            {isStarting
              ? inProgressAttempt
                ? "Resuming..."
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm font-semibold text-slate-600">
                    {status === "submitted"
                      ? `${item.scorePercent ?? 0}% score`
                      : "In progress"}
                  </div>
                  <div
                    className={`text-sm font-extrabold ${item.passed ? "text-[#1F6F5F]" : "text-rose-600"}`}
                  >
                    {status !== "submitted"
                      ? "Resume"
                      : item.passed
  ...
  return (
    <div className="space-y-4 rounded-lg border border-[#B9D8CC] bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3 rounded-lg border border-[#B9D8CC] bg-[#F7F1E8]/70 p-4">
        <div>
          ...
          <div className="mt-1 text-sm font-bold text-slate-700">
            {review.earnedPoints}/{review.totalPoints} points ·{" "}
      ...
      {(review.answers || []).map((answer, index) => (
        <div
          key={
            answer.skillModuleQuizAnswerId ||
            `${answer.skillModuleQuizQuestionId}-${index}`
          }
          className="rounded-lg border border-[#B9D8CC] bg-white p-4"
        >
          <div className="text-sm font-extrabold text-[#18332D]">
          ...
          <div
            className={`mt-2 text-sm font-extrabold ${answer.isCorrect ? "text-[#1F6F5F]" : "text-rose-600"}`}
          >
