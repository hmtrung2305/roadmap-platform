...
export default function RoadmapDetailDrawer({
  node,
  isEnrolled,
  isUpdating,
  isLoadingDetail,
  ...
  const canTrack =
    isEnrolled &&
    isManualNode &&
    node.isTrackable !== false &&
    status !== "locked";
  const resources = getSortedResources(
    normalizeArray(node.resources || node.Resources),
  );
  const resourceGroups = groupResourcesByType(resources);
  const learningModules = normalizeArray(
    node.learningModules || node.LearningModules,
  );
  const skills = normalizeArray(node.skills || node.Skills);
  const learningOutcomes = normalizeArray(
    node.learningOutcomes || node.LearningOutcomes,
  );
  const completionCriteria = normalizeArray(
    node.completionCriteria || node.CompletionCriteria,
  );
      ...
      >
        ...
        <div className="roadmap-drawer-scrollbar-hide min-h-0 flex-1 overflow-y-auto px-3.5 pb-2 pt-2.5">
          ...
          {status === "locked" && (
            <section className="rounded-md border border-slate-300 bg-slate-100 px-3 py-2.5 shadow-sm">
              ...
              <p className="mt-1.5 text-sm font-bold leading-6 text-slate-700">
                {lockedReason ||
                  "Complete the required prerequisites before starting this node."}
              </p>
            </section>
          )}
          ...
          {isComputedNode && status !== "locked" && (
            <CompactNotice>
              Progress updates automatically from related roadmap items.
            </CompactNotice>
          )}

          {skills.length > 0 && shouldShowSkills(nodeType) && (
            <Section
              title={nodeType === "phase" ? "Skill areas" : "Skills covered"}
            >
          ...
          {learningModules.length > 0 &&
            shouldShowLearningModules(nodeType) && (
              <Section
          ...
          {learningOutcomes.length > 0 &&
            shouldShowLearningOutcomes(nodeType) && (
              <PanelSection
                title={getOutcomeSectionTitle(nodeType)}
                items={learningOutcomes}
              />
            )}

          {completionCriteria.length > 0 &&
            shouldShowGenericCompletionCriteria(nodeType) && (
              <PanelSection
...
function ProjectNodeContent({ node, metadata, completionCriteria }) {
  ...
  return (
    <Section
      ...
      badge={
        node.isRequired === false ? (
          <SubtleBadge>Optional practice</SubtleBadge>
        ) : (
          <SubtleBadge>Milestone</SubtleBadge>
        )
      }
    >
...
function ChoiceOptionNodeContent({ metadata, completionCriteria }) {
  ...
  return (
    <Section
      title="Choice option guide"
      badge={<SubtleBadge>Selectable path</SubtleBadge>}
    >
      {optionUse && (
        <TextBlock title="When to choose this">{optionUse}</TextBlock>
      )}
...
function ChoiceGroupNodeContent({ metadata }) {
  ...
  if (!guidance && selectionNotes.length === 0) {
    return (
      <CompactNotice>
        Select one or more options from this group based on your goal and the
  ...
  return (
    <Section
      title="Choice guidance"
      badge={<SubtleBadge>Decision point</SubtleBadge>}
    >
      {guidance && (
        <TextBlock title="How to use this choice">{guidance}</TextBlock>
      )}
...
function PhaseNodeContent({ metadata }) {
  ...
  return (
    <Section
      ...
      badge={<SubtleBadge>Roadmap segment</SubtleBadge>}
    >
...
function RequirementTag({ isOptional }) {
  ...
  const style = isOptional
    ? REQUIREMENT_TAG_STYLES.optional
    : REQUIREMENT_TAG_STYLES.required;

  return (
    <span
      className={`inline-flex h-8 items-center rounded-md border px-2.5 text-[11px] font-extrabold ${style}`}
    >
      {label}
    </span>
  );
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={[
                  "flex w-full items-center justify-between gap-3 px-2.5 py-1.5 text-left text-sm font-semibold transition-colors hover:bg-[#EAF8F1]",
                  isSelected ? "bg-[#F3FBEF]" : "",
                ].join(" ")}
              >
  ...
  return (
    <div className="rounded-md border border-[#E3EEE8] bg-[#FAFCFB] p-2">
      <div className="mb-2 flex items-center gap-2">
        ...
        <span
          className={`rounded-md border px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-[0.12em] ${typeStyle.badge}`}
        >
          {typeStyle.label}
        </span>
      </div>
...
function ResourceRow({ resource, typeStyle }) {
  ...
  const title =
    resource.title || resource.Title || resource.name || url || "Resource";
  const provider =
    resource.provider ||
    resource.Provider ||
    resource.source ||
  ...
  const content = (
    <div>
      {title} <span className="font-bold text-slate-500">- {provider}</span>
    </div>
  );
